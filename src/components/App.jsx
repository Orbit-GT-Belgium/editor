...
export default class App extends React.Component {
    ...
    if (newStyle && newStyle.layers) {
      ...
      newStyle.layers.forEach((layer, index) => {
        if (layer.id === "" && foundLayers.has(layer.id)) {
          ...
          const error = new Error(`layers[${index}]: duplicate layer id [empty_string], previously used`);
  ...
  mapRenderer() {
    ...
    return (
      <div style={elementStyle} className="maputnik-map__container">
        {mapElement}
      </div>
    );
  }
  ...
  render() {
    ...
    const modals = (
      <div>
        ...
        />
        <SurveyModal isOpen={this.state.isOpen.survey} onOpenToggle={this.toggleModal.bind(this, "survey")} />
      </div>
    );
